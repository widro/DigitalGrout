  ...
  this.each(function() {
    ...
    var setDimensions = function(reset) {
      ...
      if (o.autoWidth) {
        ...
        styles.liSize = Math.round(tmpDivSize / o.visible);
    ...
    function go(to, settings) {
	  ...
	  // Reset variable start for controlItems
	  start = 1;	
      return curr;
    } // end go function

	/**
	*	controlItems: Function to control items auto active	
	*	Author: SmartAddons
	**/
	...
	function startrun(){
		...
		if(active.next() && active.next().length){//alert(curr +'--'+ o.scroll)
			 
			if(start % o.visible == 1)
			{
				
				var step = curr + o.scroll;
				start = 1;
				go( step );
			}
	...
	controlItems();
	/* End controlItems function*/
	
	/* Function hover items*/
	
	function hoverItems(){
		var all_li = ul.children('li');
		$.each(all_li, function(){
			$(this).hoverIntent({
				over:function(a){
					$curr_li_hover = $(this).index();
					$(all_li).removeClass('active');
					$mode_li = $curr_li_hover%o.visible;
					if($mode_li==0)
					{
						start = 1;
					}
